...
describe('Form (generic)', () => {
  ...
  it('calls onSubmit with correct data', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/för- och efternamn/i), { target: { value: 'Test User' } });
    fireEvent.change(screen.getByLabelText(/e-post/i), { target: { value: 'test@example.com' } });
    // Open the dropdown to reveal options
    fireEvent.click(screen.getByRole('button', { name: /välj aktiviteter/i }));
    // Click the option label to select the checkbox
    fireEvent.click(screen.getByText('A'));
    fireEvent.click(screen.getByRole('button', { name: /skicka in anmälan/i }));
    await waitFor(() => expect(onSubmit).toHaveBeenCalledWith({ name: 'Test User', email: 'test@example.com', selected: ['A'] }));
  });
});
